...
export default function AddDivisionDialog({
  ...
  editAbleDivision?: DivisionItem
}) {
  const [open, setOpen] = dialogController;
  const [ formState, setFormState ] = useState({
    name: editAbleDivision?.name || "",
    bn_name: editAbleDivision?.bn_name || "",
    url: editAbleDivision?.url || "",
    is_active:editAbleDivision?.is_active || false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    toast.promise(
      ...
      {
        loading: editAbleDivision
        ? "Updating Division..."
        : "Creating Division...",
        success: editAbleDivision
        ? "Division updated successfully!"
        : "Division created successfully!",
        error: editAbleDivision
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={isCreating || isUpdating}>
              {isCreating || isUpdating && <LoaderCircle className="animate-spin"/>}
              {editAbleDivision ? "Update" : "Create"}
            </Button>
          </div>
        </form>
